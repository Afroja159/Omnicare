...
class CompanyController extends GetxController {
  ...
  void fetchCompanyList() async {
    try {
      ...
      final List<Company> companiesFromHive = await CompanyHiveService.loadCompaniesFromHive();
      if (companiesFromHive.isNotEmpty) {
        print('Data loaded from Hive');
        companyList.assignAll(companiesFromHive);
      } else {
        final response = await http.get(Uri.parse('https://app.omnicare.com.bd/api/all_brands'));
