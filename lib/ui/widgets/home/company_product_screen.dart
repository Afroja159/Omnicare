  ...
  Future<void> fetchCompanyData() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final Map<String, dynamic> companyData = brands.firstWhere(
                (brand) => brand['id'] == widget.companyId,
            orElse: () => {});
        if (companyData.isNotEmpty) {
          final String name = companyData['brand_name'];
        ...
        print(
            'Failed to load company data. Status code: ${response.statusCode}');
      }
    } catch (error) {
