...
class _OrderListScreenState extends State<OrderListScreen> {
  ...
  Future<void> _fetchOrderData() async {
    try {
      ...
      print('Fetch Order Data status code: ${response.statusCode}');
      if (response.statusCode == 200) {
        final jsonData = json.decode(response.body);
        setState(() {
          orderList.clear();
          orderList = (jsonData['data'] as List).map((orderData) => OrderModel.fromJson(orderData)).toList();
      ...
      print('Error during orderlist fetch: $error');
    }
    finally {
      setState(() {
        isLoading = false;
                  ...
                  for (var index = 0; index < orderList.length; index++)
                    Column(
                      ...
                      children: [
                        InkWell(
                          onTap: (){
                            Get.to(OrderDetailsScreen(orderId: orderList[index].id, orderStatus: orderList[index].orderStatus));
                          },
                          child: Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                Column(
                                  ...
                                  children: [
                                    Text('Order ID: #OR-${orderList[index].id}'),
                                    Text('Order Date: ${orderList[index].orderDate}'),
                                    Text('Payment Status: ${orderList[index].paymentStatus}'),
                                    Text(
                                ...
                                Column(
                                  ...
                                  children: [
                                    ...
                                    Text('${orderList[index].orderStatus.toUpperCase()}',
                                      style: TextStyle(color: _getStatusColor(orderList[index].orderStatus),),
                                    ),
                                  ],
                                ),
                              ],
